...
アプリケーションは以下のキーを使用してブラウザのlocalStorageにデータを保存します：

| キー名 | 説明 | データ形式 | デフォルト値 |
|--------|------|------------|--------------|
| `todoapp-tasks` | タスクデータ | JSON配列 | `[]` |
| `todoapp-filter` | 選択中のフィルター | 文字列 (`all`/`active`/`completed`) | `all` |
| `todoapp-sort` | 名前順ソートの状態 | boolean | `false` |

### 保存タイミング

- **タスクデータ**: タスクの追加、編集、削除、完了状態の変更時
- **フィルター状態**: フィルターボタンクリック時
- **ソート状態**: 「名前順で並べ替え」チェックボックスの変更時

### データ復元

ページ読み込み時に自動的に以下の処理が実行されます：

1. 保存されたタスクデータの読み込みと表示
2. 最後に選択していたフィルターの適用
3. ソート設定の復元（チェックボックスの状態も含む）

### データクリア方法

ブラウザの開発者ツールから手動でクリアする場合：

```javascript
// すべてのアプリデータをクリア
localStorage.removeItem('todoapp-tasks');
localStorage.removeItem('todoapp-filter');
localStorage.removeItem('todoapp-sort');

// または個別にクリア
Storage.clear();  // タスクデータのみクリア
```
